...
export class ElectricClient {
  ...
  private syncStatusListeners: Set<(status: { isSynced: boolean; isSyncing: boolean; error: Error | null }) => void> = new Set();
  ...
  async initialize(config: ElectricClientConfig): Promise<void> {
    ...
    try {
      ...
      console.error('Failed to initialize ElectricClient:', error);
      this.notifySyncStatusChange({ isSynced: false, isSyncing: false, error: error as Error });
      throw error;
    }
  }
  ...
  private async subscribeToShape(
    ...
    subscription.stream.on('error', (error: Error) => {
      shapeSub.error = error;
      shapeSub.isSynced = false;
      this.notifySyncStatusChange({ isSynced: false, isSyncing: false, error });
    });

    return shapeSub;
  }

  private checkAllSynced(): void {
    let allSynced = true;
    ...
    if (allSynced) {
      this.notifySyncStatusChange({ isSynced: true, isSyncing: false, error: null });
    }
  }

  private notifySyncStatusChange(status: { isSynced: boolean; isSyncing: boolean; error: Error | null }): void {
    this.syncStatusListeners.forEach(listener => listener(status));
  }

  onSyncStatusChange(listener: (status: { isSynced: boolean; isSyncing: boolean; error: Error | null }) => void): () => void {
  ...
  getSyncStatus(): { isSynced: boolean; isSyncing: boolean; error: Error | null } {
    let isSynced = true;
    let isSyncing = false;
    ...
    for (const sub of this.subscriptions.values()) {
      if (!sub.isSynced) {
        return { isSynced: false, isSyncing: true, error: sub.error };
      }
    }
    return { isSynced: true, isSyncing: false, error: null };
  }
